...
public class MainSemanticVectorGeneration {
	...
	private Options buildOptions(){
		options=new Options();
		Option inputDir=OptionBuilder.hasArg(true).isRequired(true).create(GlobalVariables.INPUT_DIR_OPT);
		Option outputDir=OptionBuilder.hasArg(true).isRequired(false).create(OUTPUT_DIR_OPT);
		Option multiwordFile=OptionBuilder.hasArg(true).isRequired(false).create(MULTIWORD_FILE_OPT);
		Option numDimensions=OptionBuilder.hasArg(true).isRequired(false).create(NUM_DIM_OPT);
		Option numCycles=OptionBuilder.hasArg(true).isRequired(false).create(NUM_CYCLES_OPT);
